...
class atoi {
     ...
     */
	 ...
	 /* *****
	 1. long 累加 avoid overflow
	 2. substring(1)统一去符号。 neg符号位
	 3. early termination
	 * *****/
    public static int atoi(String str) {
        ...
        /*calculation*/
        int i = 0;
        for(int i = 0; i < str.length(); i++) { // calculate without sign
            char c = str.charAt(i);
            if (c >= '0' && c <= '9') {
                longRes = longRes * 10 + (c - '0');
				// early termination.
				if(neg && logRes > Integer.MAX_VALUE) return Integer.MIN_VALUE;
				if(!neg && logRes >= Integer.MAX_VALUE) return Integer.MAX_VALUE;  
